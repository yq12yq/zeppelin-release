...
/**
 * Utils class for Py4J related stuff
 */
public class Py4JUtils {

  private static final Logger LOGGER = LoggerFactory.getLogger(Py4JUtils.class);
  ...
  public static GatewayServer createGatewayServer(Object entryPoint,
    ...
    if (useAuth) {
      try {
        ...
        return
            (GatewayServer) builder.getClass().getMethod("build").invoke(builder);
      } catch (Exception e) {
        throw new IOException("Fail to create GatewayServer", e);
      }
    } else {
      return new GatewayServer(entryPoint, port);
    }
  }
  ...
  public static String createSecret(int secretBitLength) {
    ...
    byte[] secretBytes = new byte[secretBitLength / Byte.SIZE];
